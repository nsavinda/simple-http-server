...
def create_php_array(data):
    php_array = map(lambda item: f"    '{item[0]}' => '{item[1]}'", data)
    php_string = "array(\n" + ",\n".join(php_array) + "\n);"
    return php_string
    ...
    try:
        ...
        while True:
            try:
                ...
                path, params = path.split("?") if len(path.split("?")) == 2 else (path, None)
                ...
                if os.path.exists(file_path) and os.path.commonpath([ROOT_DIR, file_path]) == ROOT_DIR:
                    ...
                    file_path = next((os.path.join(file_path, index_file) for index_file in index_files if os.path.exists(os.path.join(file_path, index_file))), file_path)
